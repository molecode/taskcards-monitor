() => {
    ...
    containers.forEach((container, listIndex) => {
        ...
        cards.forEach((cardEl, cardIndex) => {
            ...
            let cardContent = '';
            const cardContentDiv = cardEl.querySelector('.board-card-content');
            if (cardContentDiv) {
                const contentEditable = cardContentDiv.querySelector('.contenteditable');
                if (contentEditable) {
                    cardContent = contentEditable.textContent.trim();
                }
            }
            ...
            if (!cardTitle) {
                if (cardContent) {
                    cardTitle = cardContent.substring(0, 100);
                } else {
